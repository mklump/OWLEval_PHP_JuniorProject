<?php
/* $Id: japanese-euc.inc.php,v 2.36 2004/08/05 11:19:50 lem9 Exp $ */
...
$strAccessDeniedExplanation = 'phpMyAdminは、MySQLサーバーに接続しようとしました。そして、サーバーは接続を拒絶しました。config.inc.phpの中のホスト、ユーザー名およびパスワードをチェックし、MySQLサーバーの管理人から与えられた情報にそれらが相当することを確かめるべきです。';
$strAction = '実行';
...
$strFileNameTemplateRemember = 'テンプレートを思い出す';
$strFixed = '固定';
...
$strHebrew = 'ヘブライ語';
$strHexForBinary = 'バイナリフィールドは16進数を使用する';
$strHome = 'メーンページへ';
...
$strInsecureMySQL = 'あなたのコンフィグレーションファイルの設定がMySQLデフォルトの特権アカウントになっています (rootユーザーでパスワードなし) 。 MySQLサーバーもこのデフォルト設定で動作しているので、侵入者に対して無防備です。このセキュリティーホールは必ず閉ざして下さい。';
$strInsert = '追加';
...
$strInstructions = '実施例';
$strInternalNotNecessary = '* InnoDBが存在する場合、内部リレーションは必要ではありません。';
...
$strOr = 'OR';
$strOverhead = 'オーバーヘッド';
$strOverwriteExisting = 'ファイルがぞんざいする場合は上書き';
...
$strSearchType = '検索条件:';
$strSecretRequired = '設定ファイルは今はヒミツパスフレーズを必要とします(blowfish_secret)。';
...
$strServerStatusUptime = 'このMySQLサーバーは %s 間動作中。 %s に起動しています。';
...
$strStrucExcelCSV = 'MS Excel用CSVデータ';
$strStrucOnly = '構造のみ';
...
$strStrucNativeExcel = '本来の MS Excel データ';  //to translate
